...
import { IoIosNotifications, IoMdHeartEmpty } from "react-icons/io";
...
//  SLICES
import { toggleProfileModalState } from "../../slices/AppMechanics";
...
const Profile = () => {
  ...
  const fetchFavoriteEvents = async (e: React.MouseEvent) => {
    try {
      e.preventDefault();
      const resp = await axios({
        url: `${BASEURL}/user_api/u_favlist.php`,
        method: "GET",
        headers: {},
        data: {
          uid: "",
  ...
  const fetchCoupons = async (e: React.MouseEvent) => {
    try {
      e.preventDefault();

      const resp = await axios({
        url: `${BASEURL}/user_api/u_couponlist.php`,
        method: "POST",
        headers: {},
        data: {
          uid: "",
    ...
    <div className="absolute top-0 flex flex-col bg-white h-[200vh] lg:w-[35%] xl:w-3/12 z-50 right-0">
      <div className="h-52 relative flex items-center justify-center w-full">
        ...
        <div className="z-10 flex flex-col items-center gap-2">
          ...
          <h5 className="text-website-purple-dark bg-white/10 px-1 rounded">
            Madison Beer
          </h5>
        </div>
        <button
